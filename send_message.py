...
def send_message(response, tg_token, tg_chat_id):
    ...
    if response['new_attempts'][0]['is_negative']:
        text_accept = (f'''У вас проверили работу '{response['new_attempts'][0]['lesson_title']}'
...
Ссылка на работу: {response['new_attempts'][0]['lesson_url']}''')
        ...
        bot.send_message(chat_id=tg_chat_id, text=f'''У вас проверили работу '{response['new_attempts'][0]['lesson_title']}'
